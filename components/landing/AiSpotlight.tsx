...
export function AiSpotlight() {
  ...
  return (
    <section id="ai-spotlight" className="py-28 bg-[#030712] relative overflow-hidden isolate border-b border-slate-800/80">
      {/* Background Glow */}
      <div className="absolute inset-0 bg-gradient-to-tr from-teal-950/30 via-slate-950 to-cyan-950/20 pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid lg:grid-cols-12 gap-10 items-center">
          ...
          <ScrollReveal direction="left" delay={0.1} className="lg:col-span-7">
            <SpotlightCard className="bg-[#0b1329] rounded-3xl border border-slate-800 shadow-2xl overflow-hidden font-sans relative">
              ...
              <div className="p-6 md:p-8 space-y-6 min-h-[380px] bg-slate-950/90">
                ...
                {typedText === fullText && (
                  <div className="flex justify-start animate-in fade-in slide-in-from-bottom-3 duration-500">
                    <div className="bg-slate-900 text-slate-200 rounded-2xl rounded-tl-sm p-6 max-w-[95%] border border-slate-800 space-y-4">
                      ...
                      <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 space-y-2 text-xs">
                        <div className="flex justify-between items-center border-b border-slate-800/80 pb-2">
                          <span className="font-bold text-white font-sora">{t.issuesTitle}</span>
                          <span className="bg-rose-500/20 text-rose-300 px-2.5 py-0.5 rounded-full font-mono text-[10px] font-bold border border-rose-500/30">
                            {t.riskLabel}
                          </span>
                        </div>
                        <div className="space-y-1 text-slate-300 pt-1">
                          <p>• <span className="text-slate-400">{t.primaryIssue}:</span> {t.primaryIssueVal}</p>
                          <p>• <span className="text-slate-400">{t.secondaryIssue}:</span> {t.secondaryIssueVal}</p>
                          <p>• <span className="text-slate-400">{t.impact}:</span> {t.impactVal}</p>
                        </div>
                      </div>
          ...
          <ScrollReveal direction="right" delay={0.2} className="lg:col-span-5">
            <SpotlightCard className="bg-slate-900/80 rounded-3xl p-8 border border-slate-800 backdrop-blur-xl shadow-xl space-y-6">
              <div className="flex items-center gap-3 border-b border-slate-800 pb-4">
                ...
                <div>
                  <h3 className="font-sora font-bold text-white text-base">
                    {isAr ? 'الخصوصية بالتصميم' : 'Privacy-By-Design Guarantee'}
                  </h3>
                  <p className="text-xs text-slate-400 font-mono">ISO 27001 & GDPR Compliant</p>
                </div>
              </div>
                ...
                {privacyGuarantees.map((p, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex items-start gap-3.5">
                      <div className="w-7 h-7 rounded-lg bg-teal-500/10 border border-teal-500/20 flex items-center justify-center shrink-0 text-teal-400 mt-0.5">
                        <Icon className="w-4 h-4" />
                      </div>
                      <div>
                        <h4 className="text-xs font-bold text-white font-sora mb-0.5">{p.title}</h4>
