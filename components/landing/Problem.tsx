    ...
    <section className="py-28 bg-[#020617] relative overflow-hidden isolate border-b border-slate-800/80">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <ScrollReveal className="text-center max-w-3xl mx-auto mb-20">
          <span className="text-xs font-bold font-mono tracking-widest uppercase text-rose-400 bg-rose-500/10 px-3.5 py-1.5 rounded-full border border-rose-500/20 mb-4 inline-block">
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-stretch">
          ...
          <ScrollReveal direction="left" delay={0.1}>
            <SpotlightCard 
              ...
              className="bg-slate-900/50 rounded-3xl p-8 border border-rose-500/20 relative space-y-6 flex flex-col justify-between h-full"
            >
              <div className="space-y-6">
                <div className="flex items-center justify-between border-b border-rose-500/20 pb-4">
                  <span className="font-sora font-bold text-rose-400 text-lg flex items-center gap-2">
                    <AlertOctagon className="w-5 h-5 text-rose-400" />
                    {isAr ? 'الطريقة التقليدية القديمة' : 'Traditional Manual Approach'}
                  </span>
                  <span className="text-xs font-mono bg-rose-500/10 text-rose-300 px-2.5 py-1 rounded-full border border-rose-500/20">
                ...
                <div className="space-y-4">
                  {painPoints.map((item, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex items-start gap-4 p-4 rounded-xl bg-slate-950/60 border border-slate-800/80">
                        <div className="w-10 h-10 rounded-lg bg-rose-500/10 border border-rose-500/20 flex items-center justify-center shrink-0 text-rose-400 mt-0.5">
                          <Icon className="w-5 h-5" />
                        </div>
                        <div>
                          <h4 className="text-sm font-bold text-white mb-1 font-sora">{item.title}</h4>
              ...
              <div className="p-4 rounded-xl bg-rose-500/5 border border-rose-500/10 text-xs text-rose-300/80 font-mono text-center mt-4">
          ...
          <ScrollReveal direction="right" delay={0.2}>
            <SpotlightCard 
              ...
              className="bg-slate-900/90 rounded-3xl p-8 border border-teal-500/30 relative space-y-6 flex flex-col justify-between shadow-2xl shadow-teal-950/40 h-full"
            >
              <div className="space-y-6">
                <div className="flex items-center justify-between border-b border-teal-500/30 pb-4">
                  <span className="font-sora font-bold text-teal-300 text-lg flex items-center gap-2">
                    <ShieldCheck className="w-5 h-5 text-teal-400" />
                    {isAr ? 'منصة ErgonoAI الذكية' : 'The ErgonoAI Platform'}
                  </span>
                  <span className="text-xs font-mono bg-teal-500/10 text-teal-300 px-2.5 py-1 rounded-full border border-teal-500/30">
                ...
                <div className="space-y-4">
                  {solutions.map((item, idx) => {
                    const Icon = item.icon
                    return (
                      <div key={idx} className="flex items-start gap-4 p-4 rounded-xl bg-slate-950/80 border border-teal-500/20">
                        <div className="w-10 h-10 rounded-lg bg-teal-500/20 border border-teal-500/40 flex items-center justify-center shrink-0 text-teal-300 mt-0.5">
                          <Icon className="w-5 h-5" />
                        </div>
                        <div>
                          <h4 className="text-sm font-bold text-white mb-1 font-sora">{item.title}</h4>
              ...
              <div className="p-4 rounded-xl bg-teal-500/10 border border-teal-500/20 text-xs text-teal-300 font-mono text-center mt-4">
