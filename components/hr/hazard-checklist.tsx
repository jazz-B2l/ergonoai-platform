...
export function HRHazardChecklist() {
  ...
  async function seedHazardsIfEmpty(organizationId: string) {
    ...
    const hazardsToInsert = [
      { category_id: catMap.get('Physical'), name: 'Inadequate office lighting or display screen glare', default_risk_level: 'MEDIUM' },
      { category_id: catMap.get('Physical'), name: 'Uncomfortable ambient temperature or strong drafts', default_risk_level: 'LOW' },
      { category_id: catMap.get('Physical'), name: 'Excessive machinery/background noise levels', default_risk_level: 'LOW' },
      
      { category_id: catMap.get('Mechanical'), name: 'Workspace seating lacking lumbar or armrest adjustability', default_risk_level: 'HIGH' },
      { category_id: catMap.get('Mechanical'), name: 'Fixed-height desks causing wrist flexion or neck lean', default_risk_level: 'MEDIUM' },
      
      { category_id: catMap.get('Fire'), name: 'Obstructed emergency exits or evacuation walkways', default_risk_level: 'CRITICAL' },
      { category_id: catMap.get('Fire'), name: 'Missing or expired hand fire extinguishers', default_risk_level: 'HIGH' },
      
      { category_id: catMap.get('Negative/Passive'), name: 'Unmarked first-aid kit or depleted emergency supplies', default_risk_level: 'MEDIUM' },
      { category_id: catMap.get('Negative/Passive'), name: 'Cables trailing across walkways causing slip/trip hazard', default_risk_level: 'HIGH' },
    ]
